...
function FileRead(file_name) {
	static dllid = external_define("file_dll.dll", "read_file", dll_cdecl, ty_string, 1, ty_string);
	return external_call(dllid, file_name);
}

/// @desc 写文件，传入文件名和文件内容，成功返回0，失败返回其他 
function FileWrite(file_name, file_content) {
	static dllid = external_define("file_dll.dll", "write_file", dll_cdecl, ty_real, 2, ty_string, ty_string);
...
function FileCopy(src_file_name, dst_file_name) {
	static dllid = external_define("file_dll.dll", "copy_file", dll_cdecl, ty_real, 2, ty_string, ty_string);
...
function FileMove(src_file_name, dst_file_name) {
	static dllid = external_define("file_dll.dll", "move_file", dll_cdecl, ty_real, 2, ty_string, ty_string);
	return external_call(dllid, src_file_name, dst_file_name);
}
